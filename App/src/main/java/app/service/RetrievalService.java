...
public class RetrievalService {
    ...
    /** Hybrid retrieval: vector similarity search followed by keyword-based reranking. */
    public List<ScoredDocument> hybridSearch(String query, int topK) {
        // Step 1: Vector similarity search — cast a wider net
        ...
        // Step 2: Rerank by combining vector rank score with keyword overlap
        List<ScoredDocument> scored = new ArrayList<>();
        Set<String> queryTerms = tokenize(query);

        for (int i = 0; i < candidates.size(); i++) {
            Document doc = candidates.get(i);
            double vectorScore = 1.0 - (i / (double) candidates.size()); // rank-based score
    ...
    /** Simple vector-only search (baseline for benchmarking). */
